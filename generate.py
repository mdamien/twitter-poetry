...
def choose_word_back(next1, next2, next3=None):
    ...
    with sqlite3.connect(DB_LOC) as con:
        ...
        if next3:
            try:
                cur.execute("select word4 from reverse_quadgrams where word1=? and word2=? and word3=? and summed_prob>=? order by summed_prob asc limit 1;", (next1, next2, next3, choose))
                res = cur.fetchone()[0]
            except TypeError:
               pass
        if not res:
            try:
                cur.execute("select word3 from reverse_trigrams where word1=? and word2=? and summed_prob>=? order by summed_prob asc limit 1;", (next1, next2, choose))
                res = cur.fetchone()[0]
            except TypeError:
                 cur.execute("select word2 from reverse_bigrams where word1=? and summed_prob>=? order by summed_prob asc limit 1;", (next1, choose))
                 res = cur.fetchone()[0]
    return res
...
def trigrams_back(seed_word, randomize=True):
    ...
    with sqlite3.connect(DB_LOC) as con:
        ...
        if randomize:
            ...
            cur.execute("select * from reverse_trigrams where word1=? order by probability desc limit 1;", (seed_word,))
            current = cur.next()
        word1 = current[0]
        word2 = current[1]
        word3 = current[2]
...
def rejoin(word_list):
    ...
    first = True
    for w in word_list:
        if first or w in name_list or w in i_words:
            w=w.title()
            first = False
        padded.append(w)
        if w[-1].isalpha():
